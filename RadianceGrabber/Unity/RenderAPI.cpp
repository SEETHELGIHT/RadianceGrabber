...
RenderAPI* RenderAPI::mRenderAPI = nullptr;

RenderAPI* RenderAPI::GetRenderAPI()
{
	return mRenderAPI;
}
void RenderAPI::InitializeAPI(UnityGfxRenderer apiType)
{
	mRenderAPI = CreateRenderAPI(apiType);
}
void RenderAPI::DestroyAPI()
{
	if (mRenderAPI)
	{
		delete mRenderAPI;
