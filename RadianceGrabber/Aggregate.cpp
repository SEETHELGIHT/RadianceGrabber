...
namespace RadGrabber
{
	...
	__host__  LinearAggregate::LinearAggregate(const FrameMutableInput* min, const FrameImmutableInput* imin)
	{
		...
		mMeshs = (MeshChunk*)MAllocDevice(mMeshCount * sizeof(MeshChunk));
		MCopy(mMeshs, imin->meshBuffer, imin->meshBufferLen * sizeof(MeshChunk), cudaMemcpyKind::cudaMemcpyHostToHost);
		MCopy(mMeshs + imin->meshBufferLen, imin->skinnedMeshBuffer, imin->skinnedMeshBufferLen * sizeof(MeshChunk), cudaMemcpyKind::cudaMemcpyHostToHost);
	}
	...
	__device__ bool LinearAggregate::Intersect(const Ray & ray, SurfaceIntersection & isect) const
	{
		...
		for (int i = 0; i < mMRCount; i++)
		{
			...
			{
				if (IntersectInternal(ray, mMRs + i, isectBuffer, lastDistance))
				{
		...
		for (int i = 0; i < mSkinnedMRCount; i++)
		{
			...
			{
				if (IntersectInternal(ray, (MeshRendererChunk*)(mSkinnedMRs + i), isectBuffer, lastDistance))
				{
					if (lastDistance < distance)
					{
						isect = isectBuffer;
	...
	__device__ bool IntersectRayAndPlane(const Ray& ray, const Vector3f& p0, const Vector3f& p1, const Vector3f& p2, SurfaceIntersection& isect, float& lastDistance, Vector3f& bc)
	{
	...
	__device__ bool LinearAggregate::IntersectInternal(const Ray & rayInWS, const MeshRendererChunk* mrc, SurfaceIntersection & isect, float& lastDistance) const
	{
	...
	__host__ LinearAggregate* LinearAggregate::GetAggregate(const FrameMutableInput* min, const FrameImmutableInput* imin)
	{
		FrameMutableInput in;
		ASSERT_IS_FALSE(cudaMemcpy(&in, deviceParam, sizeof(FrameMutableInput), cudaMemcpyKind::cudaMemcpyHostToDevice));

		LinearAggregate aggregateBuffer(min, imin), *deviceAggreatePtr;
	...
	__host__ TwoLayerBVH::TwoLayerBVH()
	{
	}

	__host__ TwoLayerBVH::TwoLayerBVH(const FrameMutableInput* min, const FrameImmutableInput* imin)
	{
		int internalNodeCount = (min->meshRendererBufferLen + min->skinnedMeshRendererBufferLen + min->lightBufferLen - 1);
	...
	__device__ bool TwoLayerBVH::Intersect(const Ray & ray, SurfaceIntersection & isect) const
	{
		/*
			TODO:: bvh traversal
		*/
